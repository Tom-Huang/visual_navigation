...
Our implementation lies all in repository under \emph{s0011}. Our implementation of optical flow visual odometry is in branch \emph{project\_eval}. Visual odometry with descriptor matching is in branch \emph{excise-05\_eval}. The report of this project is in branch \emph{report} in folder ./Report/one\_column\_version. The presentation slide is in branch \emph{project\_eval} root directory.
...
\begin{algorithm}[h]
  ...
  \begin{algorithmic}[1]
    ...
    \For{each $i\in [1,maximum\ level-1]$}  
...
As described in the previous section, we use a grid to segment the image frame and decide whether the next frame is a key frame. A basic set of parameters for the grid of our experiments is as follows:
